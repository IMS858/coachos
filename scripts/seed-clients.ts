...
interface RosterEntry {
  name: string;
  /** Current session number on the perpetual training counter. Undefined = no package. */
  trainingSessions?: number;
}

const roster: RosterEntry[] = [
  // Clients with a tracked training session number
  { name: "Nikki", trainingSessions: 24 },
  { name: "Will", trainingSessions: 12 },
  { name: "Diana", trainingSessions: 12 },
  { name: "Dan", trainingSessions: 33 },
  { name: "An", trainingSessions: 24 },
  { name: "Delois", trainingSessions: 12 },
  { name: "Gabe", trainingSessions: 24 },
  { name: "Jim", trainingSessions: 12 },
  { name: "Peyton", trainingSessions: 6 },
  { name: "Rajan", trainingSessions: 12 },
  { name: "Sarah S.", trainingSessions: 12 },
  { name: "Sarah H.", trainingSessions: 4 },
  { name: "Saman", trainingSessions: 12 },
  { name: "Suzanne", trainingSessions: 12 },
  { name: "Donovan", trainingSessions: 24 },
  { name: "Jerry", trainingSessions: 24 },
  { name: "Rich W.", trainingSessions: 12 },
  { name: "Frank M.", trainingSessions: 12 },
  { name: "Mark", trainingSessions: 24 },
  { name: "Steve", trainingSessions: 12 },
  { name: "Robin", trainingSessions: 15 },
...
function packageTierFor(sessions: number): "package_6" | "package_12" | "package_24" {
...
async function seedClient(entry: RosterEntry) {
  ...
  const hasPackage = entry.trainingSessions !== undefined;
  ...
  if (hasPackage && entry.trainingSessions !== undefined) {
    await supabase.from("plans").insert({
      client_id: userId,
      kind: "package",
      tier: packageTierFor(entry.trainingSessions),
      service_type: "training",
      status: "active",
      total_sessions:
        entry.trainingSessions <= 6 ? 6
        : entry.trainingSessions <= 12 ? 12
        : Math.max(24, entry.trainingSessions),
      current_session_number: entry.trainingSessions,
      sessions_used: entry.trainingSessions,
...
async function main() {
  ...
  for (const entry of roster) {
    ...
    const detail = entry.trainingSessions !== undefined
      ? `training pack #${entry.trainingSessions}`
