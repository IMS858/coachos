/** Manually maintained schema. Session linkage/feedback fields verified against hosted schema.
 * Unlisted tables still use the existing loose fallback; this is not a generated schema snapshot.
 */
export type Json = string | number | boolean | null | { [key: string]: Json | undefined } | Json[];
export type UserRole = "owner" | "trainer" | "client";
export type ClientStatus = "lead" | "assessment_booked" | "assessment_completed" | "active" | "paused" | "churned";
export type SessionStatus = "requested" | "scheduled" | "confirmed" | "completed" | "no_show" | "cancelled" | "late_cancelled";
export type ServiceType = "training" | "massage" | "pilates" | "recovery" | "body_comp";
export type PlanKind = "subscription" | "package";
export type PlanTier = "essentials_2x" | "standard_3x" | "premium_4x" | "recovery_monthly" | "custom" | "package_6" | "package_12" | "package_24" | "package_custom";
...
interface KnownTables {
 ...
 clients: {
  ...
  Insert: Partial<KnownTables["clients"]["Row"]> & {id:string}; Update: Partial<KnownTables["clients"]["Row"]>; Relationships: [];
 };
 plans: {
  Row: {id:string;client_id:string;kind:PlanKind;tier:PlanTier;service_type:ServiceType|null;custom_label:string|null;status:string;current_session_number:number|null;total_sessions:number|null;sessions_used:number|null;monthly_rate_cents:number|null;sessions_per_week:number|null;package_total_cents:number|null;start_date:string;end_date:string|null;expires_at:string|null;stripe_subscription_id:string|null;stripe_price_id:string|null;notes:string|null;created_at:string;updated_at:string};
...
export interface Database {
 public: {
  Tables: AllTables;
  Views: { [key:string]: {Row:LooseRow;Relationships:[]} };
  Functions: { [key:string]: {Args:Record<string,any>;Returns:any} };
  Enums: {user_role:UserRole;client_status:ClientStatus;session_status:SessionStatus;service_type:ServiceType;plan_kind:PlanKind;plan_tier:PlanTier;billing_type:BillingType};
