...
export async function generateSeriesSessions(
  ...
  // Upsert on the unique (series, scheduled_at) index to stay idempotent.
  const { error, count } = await svc
    .from("sessions")
    .upsert(rows, {
      onConflict: "recurring_series_id,scheduled_at",
      ignoreDuplicates: true,
      count: "exact",
    });

  if (error) {
    console.warn("[recurring] generate failed:", error.message);
    return 0;
  }
