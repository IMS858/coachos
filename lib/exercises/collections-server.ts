...
import { EMPTY_PRESCRIPTION, prescriptionSignature, savedPrescriptions } from "./prescription";
...
const dosageMatches = (data: unknown,input: ExerciseSetInput) => prescriptionSignature(input.canonical_ids,savedPrescriptions(data)) === prescriptionSignature(input.canonical_ids,input.prescriptions);
...
export async function saveExerciseSet(db: Pick<SupabaseClient, "from">, actorId: string, input: ExerciseSetInput, editing: boolean) {
  ...
  if (editing) {
    ...
    // A pre-upgrade open tab must not erase dosage it cannot display.
    if (input.prescriptions === undefined) {
      const previous=savedPrescriptions(prior.data.data);
      input={...input,prescriptions:Object.fromEntries(input.canonical_ids.map(id=>[id,previous[id]??{...EMPTY_PRESCRIPTION}]))};
    }
  } else if (prior.data) {
    const same = prior.data.status === "draft" && isExerciseSet(prior.data.data)
      ...
      && dosageMatches(prior.data.data,input);
  ...
  // One private row write. No mappings, approvals, notifications or publication changes.
  ...
  if (saved.error?.code === "23505") {
    ...
    if (!duplicate.error && row && row.status === "draft" && row.client_id === input.client_id && row.trainer_id === actorId
        ...
        && JSON.stringify(exerciseSetIds(row.data)) === JSON.stringify(input.canonical_ids) && dosageMatches(row.data,input)) {
