export const MAX_FUEL_PDF_BYTES=8*1024*1024;
export async function readFuelPdf(request:Request):Promise<Uint8Array>{
 ...
 const declared=request.headers.get("content-length");if(declared&&(!/^\d+$/.test(declared)||Number(declared)>MAX_FUEL_PDF_BYTES))throw Error("PDF must be no larger than 8 MB.");
 if(!request.body)throw Error("PDF body is missing.");const reader=request.body.getReader(),parts:Uint8Array[]=[];let size=0;
 try{for(;;){const {value,done}=await reader.read();if(done)break;size+=value.byteLength;if(size>MAX_FUEL_PDF_BYTES){await reader.cancel();throw Error("PDF must be no larger than 8 MB.");}parts.push(value);}}finally{reader.releaseLock();}
