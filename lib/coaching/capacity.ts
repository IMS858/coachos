...
function subtract(base:Interval[],cuts:Interval[]):Interval[]{
 ...
 for(const cut of merge(cuts)){
  ...
  for(const interval of current){
   if(cut.end<=interval.start||cut.start>=interval.end){next.push(interval);continue;}
...
export type TrainerCapacity={trainer_id:string;declared_minutes:number;blocked_minutes:number;usable_minutes:number;booked_minutes:number;open_minutes:number;booked_sessions:number;outside_declared_sessions:number;blocked_session_conflicts:number};
...
export function buildTrainerCapacity(now:Date,rules:AvailabilityRule[],blocks:TimeBlock[],sessions:CapacitySession[]):TrainerCapacity[]{
 ...
 return ids.map(trainer_id=>{
  ...
  const usableIntervals=subtract(availability,blockIntervals),usable=usableIntervals.reduce((n,i)=>n+i.end-i.start,0);
  ...
  const booked=bookedIntervals.reduce((sum,b)=>sum+usableIntervals.reduce((n,a)=>n+overlap(a,b),0),0);
  const outside=bookedRows.filter(s=>!availability.some(a=>s.start>=a.start&&s.end<=a.end)).length;
  const conflicts=bookedRows.filter(s=>blockIntervals.some(b=>overlap(s,b)>0)).length;
  return {trainer_id,declared_minutes:minutes(declared),blocked_minutes:minutes(blocked),usable_minutes:minutes(usable),booked_minutes:minutes(booked),open_minutes:minutes(Math.max(0,usable-booked)),booked_sessions:bookedRows.length,outside_declared_sessions:outside,blocked_session_conflicts:conflicts};
 });
}
