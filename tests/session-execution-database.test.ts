...
test("session exercise performance migration keeps prescription identity immutable and staff-scoped",async()=>{
 const db=new PGlite();try{
  ...
  await asActor(client);assert.equal((await db.query("select id from public.session_exercise_performance")).rows.length,1);await assert.rejects(db.query("update public.session_exercise_performance set coach_note='Client rewrite' where id=$1",[rowId]),/row-level security/);
