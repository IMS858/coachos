...
test("session exercise performance migration keeps prescription identity immutable and staff-scoped", async () => {
 ...
 try {
  await db.exec(`create role anon; create role authenticated; create schema auth;
   ...
   insert into public.clients values('${client}','${trainer}'); insert into public.exercises values('${exercise}');
   insert into public.programs values('${program}','${client}','${trainer}'); insert into public.sessions values('${session}','${client}','${trainer}','${program}','scheduled');
  ...
  await asActor(other);
  assert.equal((await db.query("select id from public.session_exercise_performance")).rows.length,0);
  await assert.rejects(db.query("insert into public.session_exercise_performance(id,session_id,client_id,program_id,prescription_key,exercise_name,recorded_by) values($1,$2,$3,$4,'x','Unauthorized',$5)",[randomUUID(),session,client,program,other]),/row-level security/);
  await asActor(client);
  const clientUpdate=await db.query("update public.session_exercise_performance set coach_note='Client rewrite' where id=$1 returning id",[rowId]);
  assert.equal(clientUpdate.rows.length,0);
  await asActor(trainer);
  assert.equal((await db.query<{coach_note:string}>("select coach_note from public.session_exercise_performance where id=$1",[rowId])).rows[0].coach_note,"Good tolerance");
  await asActor(owner);
  await db.query("update public.session_exercise_performance set coach_note='Owner corrected observation' where id=$1",[rowId]);
  for (const status of ["cancelled","late_cancelled","no_show"]) {
   await db.exec("reset role"); await db.query("update public.sessions set status=$1 where id=$2",[status,session]);
   await asActor(trainer);
   // USING removes protected rows from UPDATE; no exception is required and no trigger runs.
   const result=await db.query("update public.session_exercise_performance set coach_note='Should not change' where id=$1 returning id",[rowId]);
   assert.equal(result.rows.length,0);
   assert.equal((await db.query<{coach_note:string}>("select coach_note from public.session_exercise_performance where id=$1",[rowId])).rows[0].coach_note,"Owner corrected observation");
  }
