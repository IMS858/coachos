...
test("session exercise performance migration keeps prescription identity immutable and staff-scoped",async()=>{
 const db=new PGlite();try{
  ...
  await asActor(client);assert.equal((await db.query("select id from public.session_exercise_performance")).rows.length,1);await db.query("update public.session_exercise_performance set coach_note='Client rewrite' where id=$1",[rowId]);await asActor(trainer);assert.equal((await db.query("select coach_note from public.session_exercise_performance where id=$1",[rowId])).rows[0].coach_note,"Good tolerance");
  ...
  await db.exec("reset role; update public.sessions set status='cancelled' where id='"+session+"'");await asActor(trainer);await assert.rejects(db.query("update public.session_exercise_performance set coach_note='Should fail' where id=$1",[rowId]),/Missed or cancelled sessions/);
