...
test("coach intelligence is deterministic evidence, not a churn or medical prediction",()=>{
 const actions=buildCoachActions("22222222-2222-4222-8222-222222222222",{now,packageRemaining:1,nextSessionAt:null,lastCompletedAt:"2026-09-01T17:00:00Z",latestAssessmentAt:"2026-05-01",assessmentStatus:"complete",draftPrograms:1,activePrograms:0,savedExerciseSets:1,remoteCoachingEvidence:false});
 assert.ok(actions.some(a=>a.key==="package"));
 assert.ok(actions.some(a=>a.key==="quiet"));
 assert.ok(actions.length>=3);
 const draftOnly=buildCoachActions("22222222-2222-4222-8222-222222222222",{now,packageRemaining:null,nextSessionAt:"2026-09-26T17:00:00Z",lastCompletedAt:"2026-09-24T17:00:00Z",latestAssessmentAt:"2026-09-20",assessmentStatus:"complete",draftPrograms:1,activePrograms:0,savedExerciseSets:1,remoteCoachingEvidence:false});
...
test("coaching mode reflects recorded delivery and programming pathways",()=>{
 const base={now,packageRemaining:null,nextSessionAt:null,lastCompletedAt:null,latestAssessmentAt:null,assessmentStatus:null,draftPrograms:0,activePrograms:0,savedExerciseSets:0,remoteCoachingEvidence:false};
 ...
 assert.equal(coachingMode({...base,nextSessionAt:"2026-09-26T17:00:00Z",activePrograms:1}),"In-person");
 assert.equal(coachingMode({...base,nextSessionAt:"2026-09-26T17:00:00Z",activePrograms:1,remoteCoachingEvidence:true}),"Hybrid");
});
