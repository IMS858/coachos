...
test("capacity uses declared Pacific availability, blocks and actual booked overlap",()=>{
 ...
 assert.equal(result.declared_minutes,480);assert.equal(result.blocked_minutes,60);assert.equal(result.usable_minutes,420);assert.equal(result.booked_minutes,60);assert.equal(result.open_minutes,360);assert.equal(result.outside_declared_sessions,1);assert.equal(result.blocked_session_conflicts,0);
});
...
test("sessions overlapping explicit time blocks are conflicts, not usable capacity",()=>{
 const trainer="11111111-1111-4111-8111-111111111111";
