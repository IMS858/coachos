...
function load(relative,mocks){const filename=path.join(root,relative),source=fs.readFileSync(filename,'utf8'),out=ts.transpileModule(source,{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022,jsx:ts.JsxEmit.ReactJSX}}).outputText,loadedModule={exports:{}};new Function('require','module','exports',out)(name=>{if(Object.hasOwn(mocks,name))return mocks[name];if(name.startsWith('@/'))return load(''+name.slice(2)+'.ts',mocks);return require(name);},loadedModule,loadedModule.exports);return loadedModule.exports;}
