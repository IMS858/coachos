...
test("client PDF requires auth, ownership, and published-like status", () => {
  ...
  assert.match(source, /auth\.getUser\(\)/); assert.match(source, /if \(!user\).*401/);
...
test("client PDF never falls back to legacy base64 for non-staff", () => {
  assert.match(read("app/api/programs/[id]/pdf/route.ts"), /const encoded = isStaff \? program\.data\?\.pdf_base64 : null/);
});
...
test("published client JSON is an allowlist and clears private generator inputs", () => {
  const source = read("app/api/programs/[id]/publish/route.ts");
  const safeStart = source.indexOf("const safeData = {"); const safeEnd = source.indexOf("};", safeStart);
  assert.ok(safeStart >= 0 && safeEnd > safeStart, "safeData allowlist must exist");
  const safe = source.slice(safeStart, safeEnd);
  for (const forbidden of ["structured_program", "assessment_summary", "request_payload", "objective_summary", "coach_edits", "device_measurements", "voltra_sessions"]) assert.doesNotMatch(safe, new RegExp(forbidden));
  assert.match(source, /request_payload: null/); assert.match(source, /objective_summary: null/);
});
test("generated program publishing is locked until explicit release and all canonical approvals", () => {
  const source = read("app/api/programs/[id]/publish/route.ts");
  assert.match(source, /IMS_GENERATOR_CLIENT_RELEASE_APPROVED !== "true"/); assert.match(source, /mappings\.length !== 423/);
  assert.match(source, /fourWeekStructureIssues\(record\.data\.structured_program\)/); assert.match(source, /approvedIds\.has\(m\.matched_exercise_id\)/);
});
test("exercise review approval requires confirmed canonical mapping", () => {
  const source = read("app/api/exercise-reviews/route.ts");
  assert.match(source, /canonical_exercise_queue/); assert.match(source, /\.eq\("mapping_status","coach_confirmed"\)/);
  assert.match(source, /primary_joints_confirmed/); assert.match(source, /contraindications_confirmed/);
});
test("canonical identity mapping does not automatically grant safety approval", () => {
  ...
  assert.match(source, /safety_approved:false/); assert.match(source, /review_notes\.trim\(\)\.length<15/);
});
test("published generator page offers stored client PDF and hides raw generator inputs", () => {
  // Renderer moved intact behind a source-aware entry point. Test the actual renderer.
  const entry = read("app/programs/[id]/page.tsx");
  assert.match(entry, /LegacyProgramPage params=\{params\}/);
  assert.match(entry, /ims_library_program/); assert.match(entry, /\/library/); assert.match(entry, /deleted_at/);
  const page = read("components/programs/legacy-program-page.tsx");
  assert.match(page, /pdf_client_url, pdf_coach_url/); assert.match(page, /\(program as any\)\.pdf_client_url/);
  assert.match(page, /\/api\/programs\/\$\{id\}\/pdf/); assert.match(page, /isStaff && generated\.assessment_summary/);
  assert.match(page, /!hasGenerated && !isImsGenerator/);
  const publish = read("app/api/programs/[id]/publish/route.ts");
  assert.match(publish, /pdf_client_url/); assert.match(publish, /request_payload: null/);
});
...
test("assessment pipeline cannot drop cardio restrictions or infer surgical clearance", () => {
  ...
  assert.match(route, /includeUnverifiedSurgicalHistory\(/); assert.match(route, /generatorRichRestrictions\(a\.pain_map/);
  assert.match(route, /\.\.\.recommendedTrainingDays\(sessionsPerWeek\)/); assert.doesNotMatch(route, /\.\.\.\(ct\.primary_machine \?/);
});
