...
import { Inbox, Target, CreditCard, PackageSearch, Dumbbell, UserRoundCheck, ArrowRight } from "lucide-react";
...
export default async function ActionCenterPage() {
  ...
  const messages=messagesQ.data??[], leads=leadsQ.data??[], payments=paymentsQ.data??[], programs=programsQ.data??[], quiet=clientsQ.data??[];
  const total=messages.length+leads.length+payments.length+programs.length+quiet.length+lowBalance.length;
