...
export async function POST(request: NextRequest) {
  ...
  const rows = Object.entries(waivers).map(([type, dataUrl]) => ({
    client_id: clientId,
    waiver_type: type,
    // Each document carries its own version so they can be revised independently.
    waiver_version: WAIVER_BY_TYPE[type as keyof typeof WAIVER_BY_TYPE]?.version ?? type,
