...
export async function POST(
  ...
  // Generate the set-password link, then try to email it. The link is returned
  // either way so onboarding never depends on mail delivery — if the email
  // fails, the UI offers the link to text or hand over instead.
  ...
  try {
    ...
    if (linkErr || !linkData?.properties?.hashed_token) {
      emailError = linkErr?.message ?? "Couldn't generate a set-password link.";
    } else {
      inviteLink = `${site}/api/auth/callback?token_hash=${linkData.properties.hashed_token}&type=recovery&next=${encodeURIComponent("/set-password")}`;
        ...
        html: emailShell({
          ...
          bodyHtml: `
            ...
            <p style="margin:24px 0;">
              <a href="${inviteLink}" style="background:#1c6a9c;color:#ffffff;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;font-weight:600;">
                Set your password
              </a>
            </p>
            <p style="color:#6b7280;font-size:13px;">This link can be used once. After setting your password, sign in anytime at ${site}/login</p>
          `,
        }),
      });
    ...
    console.warn("[convert] invite failed:", emailError);
  }
