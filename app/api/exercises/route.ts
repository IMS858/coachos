...
export async function POST(request:NextRequest){
 ...
 const saved=await db.from("exercises").insert({name,ims_label:name,slug,category,movement_pattern:pattern,level:"intermediate",primary_joints:Array.isArray(b.primary_joints)?b.primary_joints.slice(0,5):[],equipment:Array.isArray(b.equipment)?b.equipment.slice(0,10):[],coaching_cues:cues,programming_notes:typeof b.description==="string"?b.description.trim().slice(0,2000)||null:null,system_tags:Array.isArray(b.system_tags)?b.system_tags.slice(0,10):[],tags:Array.isArray(b.tags)?b.tags.slice(0,15):[],video_provider:b.video_url?"upload":"placeholder",video_url:typeof b.video_url==="string"?b.video_url:null,thumbnail_url:typeof b.thumbnail_url==="string"?b.thumbnail_url:null,client_visible:false,status:"draft",created_by:user.id}).select("id,name,slug,status,client_visible").single();
