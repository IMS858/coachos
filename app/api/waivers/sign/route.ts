...
export async function POST(request: NextRequest) {
  ...
  // Defence in depth: the page guards this too, but a direct POST shouldn't be
  // able to produce a foreign key error instead of a readable message.
  const svcCheck = createServiceClient();
  const { data: clientRow } = await svcCheck
    .from("clients")
    .select("id")
    .eq("id", user.id)
    .maybeSingle();
  if (!clientRow) {
    return NextResponse.json(
      {
        error:
          "These agreements are signed by clients. Staff accounts don't have client records, so there's nothing to sign here.",
      },
      { status: 400 }
    );
  }
