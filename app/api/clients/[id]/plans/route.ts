...
export async function POST(
  ...
  try {
    ...
    if (body.kind === "subscription") {
      ...
      // The Package Terms the client signs promise a specific window (6 sessions
      // → 6 months, 48 → 18). Setting it here keeps the record and the contract
      // saying the same thing; leaving it null meant the agreement promised an
      // expiry the system never tracked.
      if (!insert.expires_at) {
        const months = packageExpiryMonths(Number(body.total_sessions));
    ...
    // Without this, anything unexpected surfaces as an opaque Vercel error ID
    // and there's nothing to act on. Return the real message instead.
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[plans/create] unhandled:", msg);
    return NextResponse.json(
      { error: "Couldn't create the plan", detail: msg },
      { status: 500 }
    );
  }
}
