...
export async function POST(
  ...
  // waivers.client_id is a foreign key to clients(id). A request created
  // against an id that has no clients row (a staff account, or a client
  // deleted since the link was sent) would otherwise fail with a raw FK error
  // at the last step, after they've signed everything.
  ...
  if (req.client_id) {
    const { data: exists } = await svc
      ...
      .eq("id", req.client_id)
      .maybeSingle();
    targetClientId = exists ? req.client_id : null;
  }
