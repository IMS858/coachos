...
export async function PATCH(
  ...
  // Pack size and price were previously fixed at creation, so a mis-entered
  // 48-pack could only be deleted and rebuilt.
  if (body.total_sessions !== undefined && Number(body.total_sessions) > 0)
    allowed.total_sessions = Number(body.total_sessions);
  if (body.package_total_cents !== undefined)
    allowed.package_total_cents = body.package_total_cents;
  ...
  const { error } = await supabase.from("plans").update(allowed as never).eq("id", id);
  if (error) {
