...
export default async function CanonicalMappingPage(){
 ...
 const [{data:rows,error:rowError},{data:exercises,error:exerciseError},{data:attestation}]=await Promise.all([
  supabase.from("canonical_exercise_queue").select("canonical_id,canonical_name,source_status,source_confidence,source_primary_joints,category,movement_pattern,equipment,mapping_status,matched_exercise_id,review_notes,review_priority,review_priority_reason,exact_candidate_id,exact_candidate_name,exact_candidate_count,safety_data_gap").order("review_priority").order("canonical_id").limit(1000),
  supabase.from("exercises").select("id,name,ims_label,primary_joints").order("name").limit(1000),
  supabase.from("exercise_catalog_attestations").select("id").eq("reviewed_by",user.id).order("reviewed_at",{ascending:false}).limit(1).maybeSingle(),
 ]);
 ...
 {rowError||exerciseError?<p role="alert" className="rounded-xl border border-divider p-5 text-sm text-status-limited">Could not load mapping queue. No changes can be saved.</p>:<CanonicalMappingWorkspace rows={rows??[]} exercises={exercises??[]} isOwner={profile.role==="owner"} catalogAttested={!!attestation}/>}
 </main></AppShell>;
}
