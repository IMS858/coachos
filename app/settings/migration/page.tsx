...
const batchColumns = "id,label,source_system,source_as_of,status,created_at";
type EvidenceRow = {id: string; record_type: string; reconciliation_status: string; dry_run_status: string | null; dry_run_reason: string | null};
...
export default async function MigrationCenter({searchParams}: {searchParams: Promise<{batch?: string}>}) {
  ...
  const records = batch ? await readCompleteEvidence<EvidenceRow>((start, end) => db.from("migration_records")
    .select("id,record_type,reconciliation_status,dry_run_status,dry_run_reason", {count: "exact"})
