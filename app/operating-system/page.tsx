...
export default async function OperatingSystemPage(){
 ...
 const b=todayBounds();
 const [clientsQ,sessionsQ,programsQ,paymentsQ,staffQ,rulesQ,campaignsQ,reviewsQ,classesQ,classEnrollQ,growth,lowPackages]=await Promise.all([
  ...
  db.from("class_occurrences").select("id,capacity,status").gte("starts_at",b.start).lte("starts_at",b.end).neq("status","cancelled"),
  db.from("class_enrollments").select("occurrence_id,status").in("status",["booked","waitlisted"]),
  loadLeadWorkspace(db),
  findLowBalancePackages(2),
 ]);
 if([clientsQ,sessionsQ,programsQ,paymentsQ,staffQ,rulesQ,campaignsQ,reviewsQ,classesQ,classEnrollQ].some(q=>q.error))throw new Error("IMS Operating System source evidence is unavailable. No health state was inferred.");
 ...
 const dueGrowth=reviews.filter(r=>r.due_on&&r.due_on<=b.date).length;const todayClassIds=new Set((classesQ.data??[]).map(x=>x.id)),todayClassEnroll=(classEnrollQ.data??[]).filter(x=>todayClassIds.has(x.occurrence_id)),todayClassBooked=todayClassEnroll.filter(x=>x.status==="booked").length,todayClassWait=todayClassEnroll.filter(x=>x.status==="waitlisted").length;
