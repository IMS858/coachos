...
export default async function ProgramsPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  let programQuery = db.from("programs").select("id,name,status,weeks,client_id,trainer_id,data,created_at").order("created_at", { ascending: false }).limit(1000);
  if (profile.data.role === "trainer") programQuery = programQuery.eq("trainer_id", user.id);
  ...
  return <AppShell><main className="mx-auto flex w-full max-w-6xl flex-col gap-5 pb-12">
    <header className="rounded-3xl bg-band p-6 text-white shadow-lg sm:p-8"><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/65">IMS / Coaching</p><h1 className="mt-2 text-4xl font-bold">Program Studio</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-white/80">{profile.data.role === "trainer" ? "Your assigned programming workspace. Build from the Exercise Library or current assessment evidence, then finish prescriptions before client release." : "Build around the client. Start with your exercise database or an assessment, then shape the finished program."}</p><div className="mt-5 flex flex-wrap gap-3"><Link href="/library" className={actions}><Dumbbell className="h-4 w-4"/>Pick from exercise library</Link><Link href="/assessments" className={actions}><Plus className="h-4 w-4"/>Create from assessment</Link></div></header>
    <section className="grid grid-cols-3 gap-3"><div className="rounded-2xl border border-divider bg-white p-4"><p className="text-3xl font-bold text-cream">{programs.length}</p><p className="text-xs text-cream-faint">{profile.data.role === "trainer" ? "My programs" : "Programs loaded"}</p></div><div className="rounded-2xl border border-divider bg-white p-4"><p className="text-3xl font-bold text-cream">{programs.filter(p=>p.status==="draft").length}</p><p className="text-xs text-cream-faint">Drafts</p></div><div className="rounded-2xl border border-divider bg-white p-4"><p className="text-3xl font-bold text-cream">{programs.filter(p=>["published","active"].includes(p.status)).length}</p><p className="text-xs text-cream-faint">Client-visible / active</p></div></section>
