...
export function ResignFlow({ types }: { types: WaiverDoc["type"][] }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setDone(true);
      router.refresh();
      // Give them a beat to see it landed before moving on.
  ...
  if (allDone) {
    return (
      <div className="rounded-lg border border-divider bg-navy-soft p-6 text-center">
        <h2 className="text-lg font-semibold text-cream">Ready to submit</h2>
        <p className="prose-ims text-sm text-cream-dim mt-1 mb-4">
          {docs.length === 1
            ? "One document signed."
            : `All ${docs.length} documents signed.`}{" "}
          Submit to file them.
        </p>
        {error && (
          <p className="text-sm text-status-limited mb-3">{error}</p>
        )}
