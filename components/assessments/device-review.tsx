...
function DeviceReviewEditor({ assessmentId, measurements, workouts }: Props) {
  ...
  return <section className="assessment-device-card rounded-2xl border p-5">
    ...
    <p className="mt-1 text-sm text-stone-600">Confirm testing context and readings before they can be considered for programming. Approval does not automatically change a program.</p>
    {(["activforce", "voltra"] as const).map(device => <div key={device} className="mt-4">
      ...
      {!entries[device].length && <p className="mt-1 text-sm text-stone-500">No readings yet.</p>}
      <div className="mt-2 grid gap-2">{entries[device].map((reading, index) =>
        <div key={str(reading.recorded_at) + index} className="rounded-xl border border-stone-200 p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div><p className="font-medium">{device === "activforce"
              ...
              <p className="text-sm text-stone-600">{device === "activforce"
            ...
            <span className="rounded-full border border-stone-200 px-2 py-1 text-xs">{str(reading.review_status)}</span>
          </div>
          {device === "voltra" && Array.isArray(reading.sets) && <div className="mt-2 text-sm text-stone-700">
          ...
          {device === "activforce" && Boolean(reading.notes) && <p className="mt-2 text-sm text-stone-600">{str(reading.notes)}</p>}
          <div className="mt-3 flex gap-2">
            <button disabled={!!busy || reading.review_status === "approved"} onClick={() => review(device, index, "approved")} className="rounded-lg bg-[#354f3d] px-3 py-1.5 text-sm font-medium text-white disabled:opacity-40">Approve</button>
            <button disabled={!!busy || reading.review_status === "rejected"} onClick={() => review(device, index, "rejected")} className="rounded-lg border border-stone-300 px-3 py-1.5 text-sm disabled:opacity-40">Reject</button>
          </div>
        </div>)}</div>
    </div>)}
