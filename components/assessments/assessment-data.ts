...
export type AssessmentData = {
  ...
  conditioning: {
    ...
    limitations: string;
    // Heart-rate recovery: the drop one minute after a hard effort. The single
    // best field marker of conditioning, and the generator doses intervals off it.
    hrr_end_hr: string;
    hrr_one_min_hr: string;
  };
  body_comp: {
    weight_lbs: string;
    body_fat_pct: string;
    ...
    waist_inches: string;
    method: string;        // bod_pod | calipers | scale | dexa
    tested_on: string;
    nutrition_strategy: string;  // fat_loss | maintenance | strength | endurance
    notes: string;
  };
...
/** Typical adult ranges, used to shade the goniometer band during assessment.
 *  Movement patterns (squat/hinge/lunge) are scored, not measured, so they are
 *  deliberately absent — they keep a free-text note instead. */
export const NORMAL_ROM: Record<string, { range: [number, number]; motion: string }> = {
  neck: { range: [70, 90], motion: "rotation" },
  shoulders: { range: [160, 180], motion: "flexion" },
  t_spine: { range: [35, 50], motion: "rotation" },
  wrists: { range: [60, 80], motion: "extension" },
  hips: { range: [110, 125], motion: "flexion" },
  knees: { range: [130, 150], motion: "flexion" },
  ankles: { range: [15, 25], motion: "dorsiflexion" },
};

export const AGE_RANGES = ["20s", "30s", "40s", "50s", "60s", "70s+"] as const;

export const BODY_COMP_METHODS = [
