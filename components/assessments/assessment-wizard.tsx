      ...
      {step === 2 && (
        <div className="flex flex-col gap-3">
          ...
          {PAIN_AREAS.map(([key, label]) => (
            <div key={key} className="grid grid-cols-[1fr_80px] gap-2 items-start">
              <div>
          ...
          {JOINTS.map(([key, label]) => (
            <div key={key} className="rounded-lg border border-divider/50 p-3">
              ...
              <div className="grid grid-cols-3 gap-2 mb-2">
                ...
                <div>
                  <label className={labelCls}>ROM (degrees)</label>
                  <Input value={data.movement_screen[key]?.rom_degrees ?? ""} onChange={(e) => upd((d) => (d.movement_screen[key] = { ...d.movement_screen[key], rom_degrees: e.target.value }))} placeholder="e.g. 120°" />
                </div>
                <div>
