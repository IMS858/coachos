    ...
    <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select className={selectCls} value={data.client.age_range} onChange={(e) => upd((d) => (d.client.age_range = e.target.value))}>
                <option value="">—</option>
            ...
            <div>
              ...
              <select className={selectCls} value={data.client.sex} onChange={(e) => upd((d) => (d.client.sex = e.target.value))}>
                <option value="">—</option>
            ...
            <div>
              <label className={labelCls}>Assessed on</label>
              <Input type="date" value={data.client.assessment_date} onChange={(e) => upd((d) => (d.client.assessment_date = e.target.value))} />
            </div>
          </div>
          <p className="text-xs text-cream-faint -mt-1">
            Age and sex drive the metabolic maths in the generated plan.
          </p>
      ...
      {step === 7 && (
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-lg border border-divider/50 p-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className={labelCls}>Peak HR</label>
                <Input value={cd.hrr_end_hr} onChange={(e) => upd((d) => (d.conditioning.hrr_end_hr = e.target.value))} placeholder="168" />
              </div>
              <div>
                <label className={labelCls}>HR at 1 min</label>
                <Input value={cd.hrr_one_min_hr} onChange={(e) => upd((d) => (d.conditioning.hrr_one_min_hr = e.target.value))} placeholder="142" />
              </div>
              <div>
                <label className={labelCls}>Drop</label>
                <div className="rounded-lg border border-divider bg-navy-elev px-3 py-2 text-sm tabular">
                  {(() => {
                    const a = parseInt(cd.hrr_end_hr, 10);
                    const b = parseInt(cd.hrr_one_min_hr, 10);
                    if (!Number.isFinite(a) || !Number.isFinite(b)) return <span className="text-cream-faint">—</span>;
                    const drop = a - b;
                    const tone = drop >= 30 ? "text-status-optimal" : drop >= 20 ? "text-status-moderate" : "text-status-limited";
                    return <span className={tone}>{drop} bpm</span>;
                  })()}
                </div>
              </div>
            </div>
      ...
      {step === 8 && (
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>Tested with</label>
              <select className={selectCls} value={bc.method} onChange={(e) => upd((d) => (d.body_comp.method = e.target.value))}>
                <option value="">—</option>
                {BODY_COMP_METHODS.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}
              </select>
            </div>
            <div>
              <label className={labelCls}>Tested on</label>
              <Input type="date" value={bc.tested_on} onChange={(e) => upd((d) => (d.body_comp.tested_on = e.target.value))} />
            </div>
          </div>
          <div>
            <label className={labelCls}>Nutrition strategy</label>
            <select className={selectCls} value={bc.nutrition_strategy} onChange={(e) => upd((d) => (d.body_comp.nutrition_strategy = e.target.value))}>
              <option value="">—</option>
              {NUTRITION_STRATEGIES.map((n) => <option key={n.value} value={n.value}>{n.label}</option>)}
            </select>
            <p className="text-xs text-cream-faint mt-1">
              Sets the calorie and protein targets in the generated plan.
            </p>
          </div>
