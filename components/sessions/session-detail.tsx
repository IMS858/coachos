...
export function SessionDetail({session,activePlans}:{session:SessionData;activePlans:ActivePlan[]}){
 const router=useRouter(),lock=useRef(false),{hasUnsavedResults}=useSessionWorkspace();
 ...
 const eligible=["scheduled","confirmed"].includes(session.status)&&Date.parse(session.scheduled_at)<=Date.now();
 ...
 {!eligible&&session.status!=="completed"&&<p className="text-sm text-cream-dim">Only an occurred, scheduled or confirmed training session can be completed here. Current status: {session.status.replaceAll("_"," ")}.</p>}
