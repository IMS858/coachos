...
import { NoShowButton } from "@/components/sessions/no-show-button";
    ...
    if (res.ok) {
      ...
      if (result.counter) {
        if (result.counter.incremented) {
          ...
          // The counter now reports where this lands against the pack, so a
          // package running out surfaces here rather than at reconciliation.
          if (c.over_limit) {
            setMessage(
              `Session #${c.session_number} — this package only had ${c.total_sessions}. They're over. Sell a renewal.`
            );
          } else if (c.exhausted) {
            setMessage(
              `Session #${c.session_number} of ${c.total_sessions} — that was their last one. Time to renew.`
            );
          } else if (typeof c.sessions_left === "number" && c.sessions_left <= 2) {
            setMessage(
              `Session #${c.session_number} — ${c.sessions_left} left on this package.`
            );
          } else {
            setMessage(
              `Session #${c.session_number} for ${session.client.full_name}.`
            );
          }
        } else {
          setMessage(
