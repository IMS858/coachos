...
import {saveResultBatch,type ResultsSaveReceipt} from "@/lib/coaching/session-close";
import {prescriptionSummary} from "@/lib/exercises/prescription";
import {useSessionWorkspace} from "./session-workspace";
import {SessionRestTimer} from "./session-rest-timer";
...
type RowState={id:string|null;version:string|null;value:PerformedValues;saved:string;error:string|null;message:string|null};
export function SessionExerciseLog({sessionId,programVersion,rows,existing,previous=[],readOnly=false}:{sessionId:string;programVersion:string;rows:SessionPlanRow[];existing:PerformanceRecord[];previous?:PerformanceRecord[];readOnly?:boolean}){
 ...
 const change=useCallback((key:string,update:(value:RowState)=>RowState)=>{const next={...stateRef.current,[key]:update(stateRef.current[key])};stateRef.current=next;setState(next);},[]);
 useEffect(()=>{setUnsavedResults(dirty.length>0||busy);return ()=>setUnsavedResults(false);},[dirty.length,busy,setUnsavedResults]);
 function edit(key:string,patch:Partial<PerformedValues>){if(lock.current||isClosing||readOnly)return;change(key,old=>({...old,value:{...old.value,...patch},message:null,error:null}));}
 const saveRows=useCallback(async(selected:readonly SessionPlanRow[]):Promise<ResultsSaveReceipt>=>{
  if(lock.current)return {ok:false,saved:0,failed:0,error:"Exercise results are already saving. Wait for confirmation before closing."};
  const edited=selected.filter(row=>{const s=stateRef.current[row.key];return performedSignature(s.value)!==s.saved;});
  if(!edited.length)return {ok:true,saved:0,failed:0};
  if(readOnly)return {ok:false,saved:0,failed:edited.length,error:"This session is read-only. Edited results were not saved."};
  lock.current=true;setBusy(true);
  try{return await saveResultBatch(edited,async row=>{
   const current=stateRef.current[row.key],actual=parsePerformedValues(current.value);
   ...
   const result=await response.json().catch(()=>null);if(!response.ok||result?.ok!==true||!result.id||!result.updated_at)throw new Error(result?.error||"Save was not confirmed. Your edits remain here.");
   change(row.key,()=>({id:result.id,version:result.updated_at,value:actual,saved:performedSignature(actual),message:"Saved to this session with history.",error:null}));
  },(row,error)=>change(row.key,old=>({...old,message:null,error})));}
  finally{lock.current=false;setBusy(false);}
 },[change,sessionId,programVersion,readOnly]);
 useEffect(()=>registerResults({save:()=>saveRows(rows)}),[registerResults,saveRows,rows]);
 ...
 {visible.map((row,index)=>{const current=state[row.key],changed=performedSignature(current.value)!==current.saved;
 const last=row.exercise_id?previous.find(r=>r.exercise_id===row.exercise_id&&r.session_id!==sessionId):undefined;
 ...
 {last&&<div className="mt-2 rounded-xl bg-surface-soft p-3 text-xs leading-5 text-cream-dim"><p>Previous completed session · {new Date(last.performed_at).toLocaleDateString("en-US",{timeZone:"America/Los_Angeles",month:"short",day:"numeric"})}</p><p>{performedSummary(last)}</p>{last.coach_note&&<p className="mt-2 whitespace-pre-wrap border-t border-divider pt-2">Previous private observation: {last.coach_note}</p>}{!readOnly&&last.load_performed.trim()&&<button type="button" disabled={busy||isClosing} onClick={()=>{if(current.value.load_performed&&current.value.load_performed!==last.load_performed&&!window.confirm("Replace the edited load with the previous session’s load?"))return;edit(row.key,{load_performed:last.load_performed});}} className="mt-2 min-h-11 rounded-lg border border-sky/30 bg-white px-3 font-semibold text-sky disabled:opacity-50">Use previous load</button>}<p className="mt-1 text-cream-faint">Reference only. Reusing a load leaves reps, sets, RPE and notes unchanged and does not save a result.</p></div>}
 <fieldset disabled={busy||isClosing||readOnly} className="mt-3 grid grid-cols-2 gap-3"><legend className="sr-only">Actually performed for {row.name}</legend>
 ...
 {!readOnly&&row.prescription.rest_seconds!==null&&row.prescription.rest_seconds>0&&<SessionRestTimer key={row.key+":"+row.prescription.rest_seconds} seconds={row.prescription.rest_seconds}/>}
 ...
 <button type="button" disabled={busy||isClosing||readOnly||!changed} onClick={()=>void saveRows([row])} className="mt-3 inline-flex min-h-12 items-center gap-2 rounded-xl bg-sky px-4 text-sm font-semibold text-white disabled:opacity-50"><Check className="h-4 w-4"/>Save actual result</button>
 </details>;})}
 {!readOnly&&<div className="sticky bottom-20 z-20 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-sky/30 bg-white p-3 shadow-lg lg:bottom-4"><p className="text-xs text-cream-dim">{busy?"Saving edited rows…":dirty.length?`${dirty.length} exercise result${dirty.length===1?"":"s"} not saved`:"No unsaved exercise edits"}</p><div className="flex flex-wrap gap-2"><button type="button" disabled={busy||isClosing||!dirty.length} onClick={()=>void saveRows(dirty)} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-sky px-4 text-sm font-semibold text-white disabled:opacity-50"><Save className="h-4 w-4"/>{busy?"Saving…":"Save edited results"}</button><a href="#session-close" className="inline-flex min-h-12 items-center rounded-xl border border-sky/30 px-3 text-sm font-semibold text-sky">Review & finish →</a></div></div>}
