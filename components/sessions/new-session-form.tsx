...
export function NewSessionForm({
  ...
  // Recurring (standing) booking. Only offered in schedule mode.
  const [repeat, setRepeat] = useState<"once" | "weekly">("once");
  // Extra weekdays beyond the one implied by the picked date (0=Sun..6=Sat).
      ...
      if (res.ok) {
        ...
        setError(
          data.detail
            ...
            : data.error ?? "Could not create standing booking."
        );
        setSubmitting(false);
      }
      return;
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          ...
          {/* Repeat (schedule mode only) */}
          {mode === "schedule" && (
            <div className="rounded-lg border border-divider bg-navy-deep/30 p-3 flex flex-col gap-3">
              <div className="flex gap-2">
                <button
                  ...
                  className={
                    ...
                    (repeat === "once"
                      ? "bg-sky text-white"
                      : "bg-navy-deep text-cream-faint hover:text-cream")
                  }
                >
                  One-time
                </button>
              ...
              {repeat === "weekly" && (
                <div className="flex flex-col gap-2">
                  <span className="text-xs text-cream-faint">
                    Repeats every week at this time. Add more days if they train
                  ...
                  <div className="flex flex-wrap gap-1">
                    {[
                      ...
                      const baseWeekday = scheduledAt
                        ? new Date(scheduledAt).getDay()
                        : -1;
                      const isBase = d.n === baseWeekday;
                      const on = isBase || extraDays.includes(d.n);
                      return (
                        <button
                          key={d.n}
                          type="button"
                          ...
                          className={
                            ...
                            (isBase ? " ring-1 ring-sky/50 cursor-default" : "")
                          }
                          title={isBase ? "From the date you picked" : ""}
                        >
                          {d.l}
                        </button>
                      );
                  ...
                  <span className="text-[11px] text-cream-faint">
                    Fills the calendar 8 weeks ahead and keeps refilling until you
                    cancel.
                  </span>
                </div>
              )}
...
function planLabel(plan: ActivePlan): string {
  ...
  switch (plan.tier) {
    ...
    case "package_custom":
      return plan.custom_label || `${plan.service_type ?? ""} custom pack`;
