...
export function LeadResearchDesk() {
  ...
  return <section className="space-y-5 rounded-2xl border border-divider bg-white p-5 sm:p-6" aria-label="Lead research desk">
    <div><p className="text-xs font-semibold uppercase tracking-wider text-sky">IMS Growth Intelligence</p><h2 className="mt-2 text-3xl font-semibold text-cream">Research Desk</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-cream-dim">Build a source-backed opportunity pipeline around Scripps Ranch. Research stays separate from real inquiries until you review and qualify it.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[[Building2,"Local partners","PT, wellness and referral relationships"],[BriefcaseBusiness,"Employers","Workplace wellness and employee training"],[Trophy,"Sports communities","Teams, clubs and performance groups"],[CalendarDays,"Community events","Public events and local activations"]].map(([Icon,title,copy]:any)=><button key={title} type="button" onClick={()=>setLane(title === "Local partners" ? "Local business and community referral partnerships" : title === "Employers" ? "Employer wellness and employee training partnerships" : title === "Sports communities" ? "Local sports organizations and adult performance communities" : "Public community events with vendor or training opportunities")} className="rounded-2xl border border-divider bg-surface-soft p-4 text-left transition hover:border-sky"><Icon className="h-5 w-5 text-sky"/><p className="mt-3 text-sm font-semibold text-cream">{title}</p><p className="mt-1 text-xs leading-5 text-cream-dim">{copy}</p></button>)}</div>
