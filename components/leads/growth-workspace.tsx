...
export async function GrowthWorkspace({ mode = "pipeline" }: { mode?: "pipeline" | "contacts" | "research" }) {
  ...
  return <AppShell><main className="mx-auto flex w-full max-w-6xl flex-col gap-5 pb-12">
    ...
    {me.role === "owner" && <Link href={mode === "research" ? "/growth#opportunities" : "/growth"} className="flex min-h-14 flex-wrap items-center justify-between gap-2 rounded-2xl border border-sky/30 bg-sky/5 p-4"><div><p className="font-semibold text-cream">Growth Center</p><p className="mt-1 text-xs leading-5 text-cream-dim">Approve / Hold / Dismiss research, own the next action, and trace campaigns to real inquiries and payment evidence.</p></div><ArrowRight className="h-5 w-5 shrink-0 text-sky"/></Link>}
