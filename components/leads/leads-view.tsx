...
export function LeadsView({ leads: initialLeads }: { leads: Lead[] }) {
  ...
  async function convertLead(id: string, hasEmail: boolean) {
    ...
    if (res.ok && data.client_id) {
      if (data.invite_sent) {
        setActionMsg("Client created — a set-password invite was emailed to them.");
        router.push(`/clients/${data.client_id}`);
        return;
      }
      // Email didn't go. Stay put and hand over the link so onboarding still
      // completes — Jason can text it or read it out.
      setInviteLink(data.invite_link ?? null);
      setInviteNote(
        data.invite_link
          ? "Client created, but the invite email didn't send. Send them this link instead."
          : `Client created, but no invite could be generated. ${data.email_error ?? ""}`
      );
      router.refresh();
    } else {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {actionMsg && (
        <div className="rounded-lg bg-status-moderate/10 border border-status-moderate/30 px-3 py-2 text-sm text-status-moderate">
          {actionMsg}
        </div>
      )}
      ...
      {inviteNote && (
        <div className="rounded-lg border border-status-moderate/40 bg-status-moderate/10 p-4 flex flex-col gap-2">
          <p className="text-sm text-cream">{inviteNote}</p>
          {inviteLink && (
            <>
              <code className="text-[11px] text-cream-dim break-all bg-navy-elev rounded p-2 border border-divider">
                {inviteLink}
              </code>
              <div className="flex gap-2">
                ...
                <Button variant="secondary" size="sm" onClick={() => { setInviteLink(null); setInviteNote(null); }}>
                  Dismiss
                </Button>
              </div>
              <p className="text-xs text-cream-faint">
                One-time use. If it expires, they can use &ldquo;Forgot your password?&rdquo; on the login page.
              </p>
            </>
          )}
        </div>
