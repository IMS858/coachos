...
import {Activity,CalendarCheck,ClipboardCheck,Dumbbell,Scale} from "lucide-react";
import {createClient} from "@/lib/supabase/server";
import {isExerciseSet} from "@/lib/exercises/catalog";

type TimelineItem={key:string;at:string;kind:"session"|"assessment"|"program"|"body";title:string;detail:string;href?:string};
const icon={session:CalendarCheck,assessment:ClipboardCheck,program:Dumbbell,body:Scale} as const;
...
export async function ClientCoachingTimeline({clientId}:{clientId:string}){
 ...
 const [sessionsQ,assessQ,programsQ,bodyQ]=await Promise.all([
