...
/** Per-session rate steps down as the pack gets bigger: $100 / $95 / $90 / $85. */
const PACKAGE_PRESETS = [
  ...
  { value: 48, label: "48-pack", priceCents: 408000 },
] as const;
...
export function ClientEditor({
  ...
  async function addPlan(payload: any) {
    ...
    return {
      ok: false,
      error: [err.error ?? "Save failed", err.detail].filter(Boolean).join(" — "),
    };
  }
...
function AddPlanPanel({
  ...
  async function handleSave() {
    ...
    if (kind === "subscription") {
      ...
      payload.tier =
        packageSize === CUSTOM_PACKAGE ? "package_custom" : `package_${packageSize}`;
      payload.service_type = serviceType;
      payload.current_session_number = sessionCount;
      payload.sessions_used = sessionCount;
      if (packageSize === CUSTOM_PACKAGE) {
        const sessions = parseInt(customSessions, 10);
        const total = parseFloat(customPackagePrice);
        if (!Number.isFinite(sessions) || sessions < 1) {
          setError("Enter how many sessions the package includes.");
          setSaving(false);
          return;
        }
        if (!Number.isFinite(total) || total < 0) {
          setError("Enter the total price for the package.");
          setSaving(false);
          return;
        }
        payload.total_sessions = sessions;
        payload.package_total_cents = Math.round(total * 100);
        if (customLabel.trim()) payload.custom_label = customLabel.trim();
      } else {
        payload.total_sessions = packageSize;
        payload.package_total_cents = PACKAGE_PRESETS.find(
          (pp) => pp.value === packageSize
        )?.priceCents;
      }
    }
            ...
            {tier === "custom" && (
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-divider">
                <div>
                  ...
                  <Input
                    placeholder="e.g. 2x/week + recovery"
                    value={customLabel}
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-1.5">
                ...
                <button
                  type="button"
                  onClick={() => setPackageSize(CUSTOM_PACKAGE)}
                  className={`rounded-md border p-3 text-center transition-colors ${
                    packageSize === CUSTOM_PACKAGE
                      ? "border-sky bg-sky/10 text-cream"
                      : "border-divider text-cream-dim hover:border-cream-faint"
                  }`}
                >
                  <div className="font-medium text-sm">Custom</div>
                  <div className="text-xs text-cream-faint mt-0.5">Any size</div>
                </button>
              </div>
            </div>
            ...
            {packageSize === CUSTOM_PACKAGE && (
              <div className="grid grid-cols-2 gap-3 pt-3 border-t border-divider">
                ...
                <div>
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-cream-faint">$</span>
                    <Input
                      type="number"
                      step="0.01"
                      placeholder="0.00"
                ...
                <div className="col-span-2">
                  ...
                  <Input
                    placeholder="e.g. Corporate 36-pack"
                    value={customLabel}
                    onChange={(e) => setCustomLabel(e.target.value)}
                  />
                  {customSessions && customPackagePrice && Number(customSessions) > 0 && (
                    <p className="text-xs text-cream-faint mt-1.5">
                      Works out to $
                      {(Number(customPackagePrice) / Number(customSessions)).toFixed(2)} per session.
                    </p>
                  )}
                </div>
              </div>
...
function planLabel(plan: Plan): string {
  ...
  switch (plan.tier) {
    ...
    case "package_48": return "48-session pack";
    case "package_custom": return "Custom package";
