...
export function CanonicalMappingWorkspace({rows,exercises}:{rows:Canonical[];exercises:Exercise[]}) {
 ...
 const missingRationale=!!selected&&notes.trim().length<15;
 ...
 return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(320px,420px)]">
  ...
  <section className="h-fit space-y-4 rounded-2xl border border-divider bg-navy-soft p-5 lg:sticky lg:top-5">{!selected?<p className="text-sm text-cream-dim">Select a canonical source exercise to confirm its matching library entry.</p>:<>
   ...
   <p className="rounded-lg border border-sky/30 p-3 text-sm text-cream-dim">To enable confirmation: (1) select the matching library movement below, and (2) write at least 15 characters explaining how you verified it. If no movement matches, leave this item pending; do not select a different exercise just to proceed.</p>
   ...
   <button type="button" disabled={busy||missingCandidate||missingRationale} onClick={save} className="min-h-11 w-full rounded-lg bg-sky px-4 text-sm font-semibold text-navy disabled:opacity-40">{busy?"Saving…":"Confirm identity mapping"}</button>
