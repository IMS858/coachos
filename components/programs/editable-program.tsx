...
/** Guard against malformed AI output so the editor never crashes on .map(). */
function normalize(d: any): ProgramData {
  const ws = Array.isArray(d?.weekly_structure) ? d.weekly_structure : [];
  return {
    ...d,
    weekly_structure: ws.map((day: any) => ({
      day_label: day?.day_label ?? "Day",
...
export function EditableProgram({
  ...
  const [data, setData] = useState<ProgramData>(() => normalize(initialData));
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between gap-3 sticky top-0 z-10 bg-navy-deep/95 backdrop-blur py-2">
        <div className="flex items-center gap-2">
          ...
          {dirty && (
            <span className="text-xs text-status-moderate">unsaved changes</span>
          )}
          {savedAt && (
        ...
        <Card key={di}>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <button
              ...
              className="text-cream-faint hover:text-status-limited ml-2"
              title="Remove day"
            >
              ...
              <div key={bi}>
                ...
                <div className="flex flex-col gap-2">
                  {blk.exercises.map((ex, ei) => (
                    <div key={ei} className="flex flex-col gap-1 border-b border-divider/30 pb-2">
                      <div className="flex items-start gap-2">
                        ...
                        <button
                          ...
                          className="text-cream-faint hover:text-status-limited mt-1"
                        >
                      ...
                      <div className="flex flex-wrap gap-2">
                        <input
                          className={`${inputCls} text-xs w-12 text-center`}
                          value={ex.sets ?? ""}
                          placeholder="sets"
                        ...
                        <input
                          ...
                          value={ex.tempo ?? ""}
                          placeholder="tempo"
                          onChange={(e) => mutate((d) => (d.weekly_structure[di].blocks[bi].exercises[ei].tempo = e.target.value))}
                        />
                        <input
                          className={`${inputCls} text-xs w-14 text-center`}
                          value={ex.rest ?? ""}
                          placeholder="rest"
                          onChange={(e) => mutate((d) => (d.weekly_structure[di].blocks[bi].exercises[ei].rest = e.target.value))}
                        />
                        <input
                          className={`${inputCls} text-xs w-20 text-center`}
                          value={ex.intensity ?? ""}
                          placeholder="RPE/effort"
                          onChange={(e) => mutate((d) => (d.weekly_structure[di].blocks[bi].exercises[ei].intensity = e.target.value))}
                        />
                      </div>
                      <input
