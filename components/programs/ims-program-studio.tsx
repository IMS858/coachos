...
export function ImsProgramStudio({ plan, programId, initialEdits, canPublish = false }: { plan: Plan; programId: string; initialEdits?: Plan | null; canPublish?: boolean }) {
  ...
  async function regenerate() {
    ...
    try {
      ...
      setSavedEdits(false); setPdfReady(true); setNotice("Client PDF regenerated. Publication still requires the release and exercise-review checks.");
      router.refresh();
    ...
    <section className="ims-program-studio space-y-5" aria-label="Generated training program">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-divider bg-navy-soft p-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" disabled={busy || dirty || !savedEdits} onClick={regenerate} className="rounded-lg bg-sky px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">Regenerate PDF</button>
        </div>
      ...
      <div className="ims-studio-panel p-4 sm:p-6">
        ...
        <div className="flex flex-wrap gap-2" role="group" aria-label="Training week">
          {weeks.map((w, i) => (
            <button key={i} type="button" aria-pressed={selectedWeek === i}
            ...
            <div className="grid gap-2">
              {(block.exercises ?? []).map((ex, ei) => <details key={ei} className="ims-exercise-row group">
                ...
                <div className="space-y-2 border-t border-divider px-3 py-3 text-sm text-cream-dim">
                  <label className="block"><span className="mb-1 block text-xs text-sky">Exercise name</span><input disabled={busy} className="w-full rounded-lg border border-divider bg-navy-elev p-2 text-cream" value={ex.name ?? ""} onChange={e=>editExercise(bi,ei,"name",e.target.value)} /></label>
                  <label className="block"><span className="mb-1 block text-xs text-sky">Dose · sets / reps / time</span><input disabled={busy} className="w-full rounded-lg border border-divider bg-navy-elev p-2 text-cream" value={ex.dose ?? ""} onChange={e=>editExercise(bi,ei,"dose",e.target.value)} /></label>
                  <label className="block"><span className="mb-1 block text-xs text-sky">Tempo</span><input disabled={busy} className="w-full rounded-lg border border-divider bg-navy-elev p-2 text-cream" value={ex.tempo ?? ""} onChange={e=>editExercise(bi,ei,"tempo",e.target.value)} /></label>
                  <label className="block"><span className="mb-1 block text-xs text-sky">Why this exercise</span><textarea disabled={busy} className="w-full rounded-lg border border-divider bg-navy-elev p-2 text-cream" value={ex.rationale ?? ""} onChange={e=>editExercise(bi,ei,"rationale",e.target.value)} /></label>
                  <label className="block"><span className="mb-1 block text-xs text-sky">Progression notes</span><textarea disabled={busy} className="w-full rounded-lg border border-divider bg-navy-elev p-2 text-cream" value={ex.progression_note ?? ""} onChange={e=>editExercise(bi,ei,"progression_note",e.target.value)} /></label>
