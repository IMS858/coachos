...
import { Goniometer } from "@/components/progress/goniometer";
...
export function ProgressReportView({
  ...
  // The goniometer reads a percentage metric as an opening angle.
  const headline =
    report.metrics.find((m) => m.key === "movement_quality" && m.current !== null) ??
    report.metrics.find((m) => m.unit === "%" && m.current !== null) ??
    null;
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Headline — the goniometer reads the client's own range as an angle */}
      <Card>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-6 mt-4">
            {headline && (
              <div className="shrink-0 mx-auto sm:mx-0">
                <Goniometer
                  fraction={(headline.current ?? 0) / 100}
                  baselineFraction={
                    headline.baseline !== null ? headline.baseline / 100 : null
                  }
                  label={headline.label}
                  value={`${headline.current}%`}
                  caption={
                    headline.baseline !== null &&
                    headline.baseline !== headline.current
                      ? `opened from ${headline.baseline}%`
                      : null
                  }
                  tone={forClient ? "light" : "dark"}
                />
              </div>
            )}
            ...
            <div className="min-w-0">
              <p
                className="tabular text-2xl text-cream leading-tight"
              ...
              {report.assessmentCount > 1 && (
                <p className="text-sm text-cream-dim mt-1">
                  {report.assessmentCount} assessments on record
                </p>
              )}
              {span && (
                <p className="text-xs text-cream-faint mt-1">{span}</p>
              )}
              <p className="prose-ims text-sm text-cream-dim mt-3">
                {forClient
                  ? "The dial reads your movement quality. The dotted needle is where you started."
                  : "Dial reads movement quality; dotted needle marks the baseline assessment."}
              </p>
            </div>
          </div>
        </CardContent>
            ...
            <Card key={m.key}>
              ...
              <CardContent>
                <div className="flex items-end justify-between">
                  <div>
                    <div className="tabular text-4xl font-bold text-cream" style={{ fontFamily: "var(--font-display)" }}>
                      {m.current ?? "—"}
                ...
                <p className="prose-ims text-xs text-cream-faint mt-3">{m.description}</p>
              </CardContent>
            </Card>
          );
        })}
