  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-divider">
      ...
      <div
        className={
          hasPhoto
            ? "absolute inset-0 bg-gradient-to-r from-[#17191c] via-[#17191c]/80 to-[#17191c]/20"
            : "absolute inset-0 bg-gradient-to-br from-sky-deep via-[#17191c] to-[#17191c]"
        }
      />

      {/* Subtle accent glow */}
      <div className="absolute -top-16 -right-16 h-48 w-48 rounded-full bg-sky/20 blur-3xl" />

      <div className="relative px-6 py-8 sm:px-8 sm:py-10">
        <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight text-white">
          {greeting}
        </h1>
        <p className="text-sm text-white/75 mt-1.5">{subline}</p>
      </div>
    </div>
  );
}
