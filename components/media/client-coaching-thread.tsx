...
export async function ClientCoachingThread({clientId}:{clientId:string}){
 ...
 const query=await db.from("client_media").select("id,title,kind,note,created_at,review_status,coach_feedback,reviewed_at").eq("client_id",clientId).eq("uploaded_by",clientId).like("storage_path",clientId+"/from-client-%").is("archived_at",null).order("created_at",{ascending:false}).limit(21);
 ...
 return <section id="form-video-review" aria-label="Form videos and coach feedback" className="scroll-mt-24 rounded-3xl border border-sky/20 bg-white p-5 shadow-sm">
  ...
  </article>)}</div>:<p className="mt-4 rounded-xl bg-surface-soft p-4 text-sm text-cream-dim">No client-submitted clips are recorded yet. {isStaff?"Ask the client to send a movement from My Plan.":"Send a clip above when you need technique feedback."}</p>}
  {(query.data??[]).length>20&&<p className="mt-3 text-xs text-cream-faint">Showing the latest 20 submissions. Older entries remain in the coaching record.</p>}
