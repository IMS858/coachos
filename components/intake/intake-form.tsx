    ...
    <div>
      ...
      <div className="rounded-xl bg-white border border-line p-6">
        ...
        {stepKey === "identity" && (
          <IdentityStep
            ...
            onChange={(f: string, v: string) => updateField("identity", f, v)}
          />
        )}
        {stepKey === "health" && (
          <HealthStep
            data={responses.health}
            onChange={(f: string, v: string) => updateField("health", f, v)}
          />
        )}
        {stepKey === "movement" && (
          <MovementStep
            data={responses.movement}
            onChange={(f: string, v: string) => updateField("movement", f, v)}
          />
        )}
        {stepKey === "goals" && (
          <GoalsStep
            data={responses.goals}
            onChange={(f: string, v: string) => updateField("goals", f, v)}
          />
        )}
        {stepKey === "logistics" && (
          <LogisticsStep
            data={responses.logistics}
            onChange={(f: string, v: string) => updateField("logistics", f, v)}
          />
        )}
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-1.5 text-sm text-ink/60 hover:text-ink disabled:opacity-30 disabled:cursor-not-allowed"
        >
        ...
        {currentStep < STEPS.length - 1 ? (
          <button
            type="button"
            onClick={next}
            className="flex items-center gap-1.5 rounded-md bg-sky text-white text-sm font-medium px-5 py-2.5"
          >
            Next
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full rounded-md border border-line bg-paper px-3 py-2 text-sm text-ink placeholder:text-ink/40 focus:outline-none focus:ring-2 focus:ring-sky"
      />
    </div>
  );
}
...
function YesNo(props: {
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <span className="text-sm text-ink/80 flex-1">{props.label}</span>
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${
                selected
                  ...
                  : "bg-paper-deep text-ink/60 hover:bg-paper-deep/80"
              }`}
            >
              {opt}
            </button>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="sm:col-span-2 mt-2 pt-4 border-t border-line">
        <h3 className="text-sm font-semibold text-ink mb-3">Emergency Contact</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <div className="space-y-4">
      <div>
        ...
        <select
          ...
          className="w-full rounded-md border border-line bg-paper px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-sky"
        >
