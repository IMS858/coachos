...
export type FuelMutationState={submit:(input:unknown)=>Promise<void>;retry:()=>Promise<void>;disabled:boolean;uncertain:boolean;busy:boolean;message:string|null;error:string|null};
export function useFuelMutation(clientId:string):FuelMutationState{
 const router=useRouter(),lock=useRef(false),pending=useRef<FuelCommand|null>(null),ambiguous=useRef(false),completed=useRef(false);
 ...
 async function send(command:FuelCommand){
  if(lock.current||completed.current)return;lock.current=true;setState("busy");setError(null);setMessage(null);
  try{const response=await fetch(`/api/fuel/${clientId}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(command)});const body=await response.json().catch(()=>null);
   if(!response.ok){if(response.status>=500||ambiguous.current){ambiguous.current=true;setState("uncertain");setError(body?.error??"Save is unconfirmed. Retry the same request.");}else{pending.current=null;setState("idle");setError(body?.error??"Save rejected. Refresh and review your inputs.");}return;}
   if(!validFuelReceipt(body,command,clientId)){ambiguous.current=true;setState("uncertain");setError("Incomplete save receipt. Retry the preserved request before editing.");return;}
   pending.current=null;ambiguous.current=false;completed.current=true;setState("saved");setMessage("Saved and confirmed. Refreshing this record. No automatic nutrition adjustment, payment or message was sent.");router.refresh();
  }catch{ambiguous.current=true;setState("uncertain");setError("Connection interrupted. Your request is preserved; retry without changing it.");}finally{lock.current=false;}
 }
 async function submit(input:unknown){if(lock.current||pending.current||completed.current)return;let command;try{command=parseFuelCommand(input);}catch(cause){setError(cause instanceof Error?cause.message:"Invalid fuel inputs.");return;}pending.current=command;await send(command);}
 return {submit,retry:()=>pending.current?send(pending.current):Promise.resolve(),disabled:state!=="idle",uncertain:state==="uncertain",busy:state==="busy",message,error};
}
export function MutationStatus({mutation}:{mutation:FuelMutationState}){return <>{mutation.error&&<p role="alert" className="rounded-xl border border-status-limited/30 bg-white p-3 text-sm text-status-limited">{mutation.error}</p>}{mutation.message&&<p role="status" className="rounded-xl bg-sky/5 p-3 text-sm text-sky">{mutation.message}</p>}{mutation.uncertain&&<button type="button" className={secondaryClass} onClick={()=>void mutation.retry()}>Retry preserved request</button>}</>;}
