 ...
 */
// Public means "don't redirect to login" — NOT "no authorisation". Every API
// route below still enforces its own: /api/agreements/send requires staff and
// returns 401 otherwise, while /api/agreements/[token] is authorised by the
// token itself. Listing the prefix here only stops middleware bouncing an
// unauthenticated signer who legitimately has no account.
const PUBLIC_ROUTES = [
  "/login",
  "/forgot-password",
  "/api/auth/callback",
